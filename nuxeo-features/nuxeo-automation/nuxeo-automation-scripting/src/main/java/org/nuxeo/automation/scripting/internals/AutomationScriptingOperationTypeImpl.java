 ...
 * @since 7.2
 */
public class AutomationScriptingOperationTypeImpl implements OperationType {

    protected final AutomationScripting scripting;

    protected final AutomationService automation;

    protected final AutomationScriptingOperationDescriptor desc;

    protected final InvokableMethod method = runMethod();

    public AutomationScriptingOperationTypeImpl(AutomationScriptingImpl scripting, AutomationService automation,
            AutomationScriptingOperationDescriptor desc) throws ScriptingException {
    ...
    public Object newInstance(OperationContext ctx, Map<String, ?> args) throws OperationException {
       return new AutomationScriptingOperation(desc.source, ctx, resolve(ctx, args));
    }
...
//    }
    ...
    public Class<?> getType() {
        return AutomationScriptingOperation.class;
    }
    ...
    protected InvokableMethod runMethod() {
        try {
            return new InvokableMethod(this, AutomationScriptingOperation.class.getMethod("run"));
        } catch (ReflectiveOperationException cause) {
            throw new Error("Cannot reference run method of " + AutomationScriptingOperation.class);
        }
    }
