...
public class TestCompileAndContext {
    ...
    public void serviceShouldBeDeclared() throws Exception {
        try (AutomationScripting.Context ref = pool.get(session)) {
    ...
    public void testNashornWithCompile() throws Exception {
        try (AutomationScripting.Context ref = pool.get(session)) {
            ...
            assertEquals("1" + System.lineSeparator() + "str" + System.lineSeparator() + "[1, 2, {a=1, b=2}]"
                    ...
                    + System.lineSeparator() + "done" + System.lineSeparator(), output.toString());
        }
    }
    ...
    public void checkScriptingEngineCostAndIsolation() throws Exception {
        try (AutomationScripting.Context session = pool.get(this.session)) {
    ...
    public class Mapper {
        ...
        final
        public Object callMe(ScriptObjectMirror params) throws IOException {
    ...
    public void testIsolationScriptCtx() throws Exception {
        ...
        try (AutomationScripting.Context scripting1 = pool.get(session)) {
            ScriptObjectMirror mirror1 = (ScriptObjectMirror)scripting1.adapt(ScriptContext.class).getAttribute("Repository");
            try (AutomationScripting.Context scripting2 = pool.get(session)) {
                ScriptObjectMirror mirror2 = (ScriptObjectMirror)scripting2.adapt(ScriptContext.class).getAttribute("Repository");
    ...
    public void testAutomationCtxSharing() throws Exception {
        ...
        try (AutomationScripting.Context scripting = pool.get(session)) {
