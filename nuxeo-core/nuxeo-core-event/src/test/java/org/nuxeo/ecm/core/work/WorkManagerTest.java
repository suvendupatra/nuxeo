...
public class WorkManagerTest {
    ...
    protected WorkQueueMetrics getCurrentMetrics() {
    	return service.getMetrics(QUEUE);
    }

    protected void assertMetricsDiff(WorkQueueMetrics initial, long scheduled, long running, long completed, long canceled) {
    	long actualScheduled = initial.getScheduled().longValue()+scheduled;
    	long actualRunning = initial.getRunning().longValue()+running;
    	long actualCompleted = initial.getCompleted().longValue()+completed;
    	long actualCanceled = initial.getCanceled().longValue()+canceled;
    	assertMetrics(actualScheduled, actualRunning, actualCompleted, actualCanceled);
    }
